// Returns signed copy requests from Supabase `signed_copy_requests` table.
...
exports.handler = async (event) => {
  ...
  try {
    let url = `${SUPABASE_URL}/rest/v1/signed_copy_requests?select=id,member_name,member_email,book_title,author_name,notes,status,created_at&order=created_at.desc&limit=500`;
    ...
    const rows = await res.json();
    return json({ success: true, requests: Array.isArray(rows) ? rows : [] });
  } catch (err) {
