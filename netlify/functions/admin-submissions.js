// Returns all book submissions from Supabase, optionally filtered by status.
// POST { adminEmail, adminKey, status? }

const crypto     = require('crypto');
const SUPABASE_URL  = process.env.SUPABASE_URL;
const SUPABASE_KEY  = process.env.SUPABASE_KEY;
// Service role key bypasses RLS — needed to fetch all rows as admin
...
exports.handler = async (event) => {
  ...
  try {
    let url = `${AUTH_SB_URL}/rest/v1/submissions?order=submitted_date.desc&limit=200`;
