// Adds the 'author' tag to a Shopify customer so they qualify for the
// AUTHOR20 discount segment ("customer_tags CONTAINS 'author'").
// Called fire-and-forget after a successful Author's Space login.
//
// Flow:
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (existing.includes('author')) return json({ success: true, alreadyTagged: true });

    // ── Step 3: PUT merged tag list ───────────────────────────────────────────
    const putRes = await fetch(adminUrl, {
      method:  'PUT',
      headers: { ...adminHeaders, 'Content-Type': 'application/json' },
      body:    JSON.stringify({ customer: { id: numericId, tags: [...existing, 'author'].join(', ') } })
    });
